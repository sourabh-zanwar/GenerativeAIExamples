...
def build_page(client: chat_client.ChatClient) -> gr.Blocks:
    ...
    with gr.Blocks(title=TITLE, theme=kui_theme, css=kui_styles + _LOCAL_CSS) as page:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=10, min_width=150, visible=False):
                tts_checkbox = gr.Checkbox(
                    label="Enable TTS output", info="", value=True
                )

        # dropdowns
        with gr.Accordion("ASR and TTS Settings", visible=False):
            with gr.Row():
                ...
                try:
                    ...
                    default_voice = tts_utils.TTS_MODELS[tts_language_list[0]][
                        "voices"
                    ][0]
                except:
        ...
        with gr.Row():
            with gr.Column(scale=10, min_width=250):
                msg = gr.Textbox(
                    show_label=False,
        ...
        audio_mic.stop_recording(
            asr_utils.stop_recording, [state], [state], api_name=False
        )
        audio_mic.stream(
...
def _stream_predict(
    ...
    for chunk in client.predict(
        query=question, use_knowledge_base=use_knowledge_base, num_tokens=OUTPUT_TOKENS
    ):
        if chunk:
            chunks += chunk
